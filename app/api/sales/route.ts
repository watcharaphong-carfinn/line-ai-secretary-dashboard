...
// งานเซล · ส่งงาน — อ่าน aggregate จาก marketing/summary (ไม่มี PII, ไม่มีงบ/รายได้การตลาด)
//   สิทธิ์แยกจากการตลาด: gate("sales") — คนมีสิทธิ์เซลไม่จำเป็นต้องเห็น ROAS/งบโฆษณา
//   เขียนโดย syncLeads() ในบอท (line-ai-secretary) — field: agg (byLeasing/byAgent/byReason ...)
  ...
  try {
    ...
    if (res.status === 404) {
      return Response.json({
        agg: null, leadCount: 0, updatedAt: null,
        note: 'ยังไม่มีข้อมูลงานเซล — สั่ง "force sync" ในไลน์เพื่อดึงรอบแรก',
      });
    }
